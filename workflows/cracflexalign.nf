...
//
// MODULES: Local modules specific to this pipeline
//
...
include { PYGTF2SGR; PYGTF2SGR as SECONDPYGTF2SGR } from '../modules/local/pygtf2sgr'
include { PYGTF2BEDGRAPH; PYGTF2BEDGRAPH as SECONDPYGTF2BEDGRAPH } from '../modules/local/pygtf2bedgraph'
...
workflow CRACFLEXALIGN {
    ...
    ch_fastqc_process = INPUT_CHECK.out.reads
    ch_fastqc_input = ch_fastqc_process
                        .groupTuple(by: [1])
                        .map { meta , fastq -> [[id: fastq.simpleName], fastq]}
    ...
    //
    // MODULE: run HISAT2_BUILD - generation of alignement index for downstream HISAT2_ALIGN step
    //
    ...
    //
    // MODULE: run BOWTIE2_BUILD - generation of alignement index for downstream BOWTIE2_ALIGN step
    //
    ...
    ch_fasta_bowtie2     = Channel.from( file(params.fasta) ).map { row -> [[id:"bowtie2_fasta"], row]}
        ...
        ch_bt2_index_single = BOWTIE2_BUILD( ch_fasta_bowtie2 ).index
    ...
    ch_alignment_out   = Channel.empty()
    if (params.aligner == 'star'){
        STAR_ALIGN( 
    ...
    //
    // MODULE: run HISAT2_ALIGN - alignment of previously processed fastq files to index generated by HISAT2_BUILD
    //
    ...
    if (params.aligner == 'hisat2'){
        ...
        ch_hisat2_multiqc = HISAT2_ALIGN.out.summary
    }

    //
    // MODULE: run BOWTIE2_ALIGN - alignment of previously processed fastq files to index generated by BOWTIE2_BUILD
    //
    ...
    if (params.aligner == 'bowtie2'){
        ...
        ch_bw2_index_process = ch_collapsed_out
                                .combine(ch_bt2_index_single)

        ch_bt2_index = ch_bw2_index_process.map { meta , fastq, meta2, index -> [meta2, index]}
        ...
        ch_bowtie2_multiqc = BOWTIE2_ALIGN.out.log
    }
    ...
    ch_first_pyread_out = PYREADCOUNTERS.out.mapped
    //
    ...
    ch_first_pyread_postprocess = ch_first_pyread_out  
                                    .combine(ch_chromosome)

    ch_second_pyread_postprocess = ch_second_pyread_out 
                                    .combine(ch_chromosome)
    ...
    PYGTF2SGR(
        ch_first_pyread_postprocess
    )

    SECONDPYGTF2SGR(
        ch_second_pyread_postprocess
    )
